...
class BasketGenerator:
    ...
    def generate_basket(self, order_date, start_date, end_date, customer_behavior=None):
        ...
        while current_total < target_value * 0.9 and attempts < max_attempts:
            ...
            # Select product (popular products more likely)
            # Use exponential distribution to favor products at the beginning of the list
            idx = min(int(np.random.exponential(len(tier_products) / 3)), len(tier_products) - 1)
