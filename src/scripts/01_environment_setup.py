...
PRODUCTS_DB = DATA_DIR / "products.duckdb"
ORDERS_DB = DATA_DIR / "orders.duckdb"
CUSTOMERS_CSV = project_root / "customers.csv"  # Will need to verify location

def check_environment():
    """Check if all required packages are installed"""
    required_packages = [
        'pandas', 'numpy', 'duckdb', 'matplotlib', 
        'datasetsforecast', 'tqdm'
    ]
...
def main():
    ...
    if all_valid:
        ...
        config = {
            ...
            'customers_csv': str(customers_path) if customers_path else None,
            'data_dir': str(DATA_DIR),
            'setup_timestamp': datetime.now().isoformat()
        }
        
        import json
        config_path = project_root / "src" / "scripts" / "config.json"
